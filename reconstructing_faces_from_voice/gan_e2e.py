...
from network import get_network, get_combined_network
...
ge_net, ge_optimizer = get_combined_network('eg', NETWORKS_PARAMETERS, train=True)
...
real_label = torch.full((DATASET_PARAMETERS['batch_size'], 1), 1).to(torch.float32)
fake_label = torch.full((DATASET_PARAMETERS['batch_size'], 1), 0).to(torch.float32)
...
for it in range(100000):
    ...
    if it % 200 == 0:
        ...
        # snapshot
        save_model(ge_net, NETWORKS_PARAMETERS['eg']['model_path'])
