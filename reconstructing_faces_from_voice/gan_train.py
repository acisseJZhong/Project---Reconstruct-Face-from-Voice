...
e_net, e_optimizer = get_network('e', NETWORKS_PARAMETERS, train=True)
...
for it in range(100000):
    ...
    D_real_loss = F.binary_cross_entropy(torch.sigmoid(real_score_out.to(dtype=torch.float64)), real_label.to(dtype=torch.float64))
    D_fake_loss = F.binary_cross_entropy(torch.sigmoid(fake_score_out.to(dtype=torch.float64)), fake_label.to(dtype=torch.float64))
    ...
    GD_fake_loss = F.binary_cross_entropy(torch.sigmoid(fake_score_out.to(dtype=torch.float64)), real_label.to(dtype=torch.float64))
